...
void HEAP_SwapNodes ( Heap* H, int Index1, int Index2 )
{
	...
	HeapNode* Temp = (HeapNode*)malloc(CopySize); 				// Temp는 HeapNode 구조체 크기 만큼의 배열 (데이터 하나 들어갈 수 있는)
	
	memcpy( Temp, &H->Nodes[Index1], CopySize ); 				// Temp 배열에 Index1까지 값들 카피
	memcpy( &H->Nodes[Index1], &H->Nodes[Index2], CopySize );	// 원래 배열 Index1따지 Index2 값들을 저장
	memcpy( &H->Nodes[Index2], Temp, CopySize );				// 남은 Index2 값들까지 Temp 값들 복사
