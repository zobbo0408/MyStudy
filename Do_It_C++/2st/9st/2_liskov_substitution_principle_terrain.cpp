...
class terrain {
	...
	void set_starget_location(int x, int y){ start_location_x = x; start_location_y = y; };
...
void forest_terrain::bost_monster(monster* mon){
	if(monster_a_type == mon->get_monster_type()){
		cout << "Monster A가 숲에서는 힘이 더욱 강해집니다." << endl;
	}
}
...
void cyber_terrain::allocate_monster(monster* mon){
	update_monster_list(mon);
	cout << "모든 Monster를 사이버 공간에 배치 합니다." << endl;
}

void cyber_terrain::bost_monster(monster* mon){
	cout << "모든 Monster가 사이버 공간에서는 속도가 더욱 빨라집니다." << endl;
}
...
void urban_terrain::bost_monster(monster* mon){
	if(monster_c_type == mon->get_monster_type()){
		update_monster_list(mon);
		cout << "Monster C는 도심에서 힘이 강해 집니다." << endl;
	}
}

// static member function, static member variable
...
monster* monster_factory::get_monster(int index){
	...
	advance(item, index); // iterator를 원하는 위치로 옮길 수 있는 함수
	new_mon = *item;
...
void monster_factory::create_terrain(){
	...
	for(auto object : terrain_inst){
		object->set_starget_location(dis(gen), dis(gen));
...
void monster_factory::create_monster(const int terrain_type, int count){
	...
	for (int i=0; i<count; ++i){
		mon = monster_factory::create_monster(terrain_type, terrain_inst[select_t(gen)]);
