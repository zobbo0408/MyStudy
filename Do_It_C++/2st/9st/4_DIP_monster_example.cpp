...
- 상위 수준의 모듈은 하위 수준의 모듈에 의존해서는 안되며,
	상위와 하위 수준은 모두 추상레이어(인터페이스)에 의존해야 한다.
*/
...
class IWeapon {
	...
	virtual void shoot_weapon(void* target_player) = 0;
};
...
class terrain {
	...
	void set_end_location (int x, int y) { end_location_x; end_location_y = y; };
...
class gun : public IWeapon {
	...
	virtual void shoot_weapon(void* target_player) override;
};
...
// 반격을 막아내는 함수, 무기와 탈 것은 모두 인터페이스 (추상 클래스)로 처리
// 인터페이스를 상속한 클래스가 무엇이던 이 함수는 변경되지 않음. 
...
bool gun::is_bullet_empty(){
	...
	return true; // 예제에서는 임의로 비었다고 가정
}

void gun::shoot_weapon(void* target_player){
	cout << "\"탕\"" << endl;
}
...
void machine_gun::shoot_weapon(void* target_player){
	cout << "\"두두두두두두\"" << endl;
}
...
void arrow::shoot_weapon(void* target_player){
	cout << "\"피이이이잉\"" << endl;
}
