/*
ISP 원칙 (Interface Segregation Principle)
인터페이스 (c++에서는 추상클래스)는 fine grained해야 하며, 
클래스는 필요한 인터페이스만 구현해야 한다. 
*/
...
uniform_int_distribution<int> select_t(0, 2);
...
int player::get_location(bool x){
	if(x){
		return location_x; // true가 매개일때
	}
	
	return location_y;
}

// IRoute, IAttack을 상속 받아 정의된 몬스터 클래스
...
void forest_terrain::bost_monster(monster* mon){
	if(monster_a_type == mon->get_monster_type()){
		cout << "Monster A가 숲에서는 힘이 더 강해 집니다." << endl;
	}
}
...
void cyber_terrain::allocate_monster(monster* mon){
	update_monster_list(mon);
	cout << "모든 종류의 Monster를 사이버 공간에 배치합니다." << endl;
}

void cyber_terrain::bost_monster(monster* mon){
	cout << "모든 Monster가 사이버 공간에서는 속도가 빨라 집니다." << endl;
}
...
terrain* character_factory::terrain_inst[] = { nullptr, };

// Monster A는 추상클래스 Monster 클래스로부터 상속
...
// Monster B는 추상클래스 Monster 클래스로부터 상속
...
// Monster C는 추상클래스 Monster 클래스로부터 상속
...
void character_factory::destroy_npc(){
	for(auto npc : npc_list){
		delete npc;
	}
}

monster* character_factory::create_monster(const int terrain_type, terrain* terrain_inst){
	monster* new_mon = nullptr;
	switch(terrain_type){
...
void character_factory::create_monster(const int terrain_type, int count){
	...
	for(int i=0; i<count; i++){
		mon = character_factory::create_monster(terrain_type, terrain_inst[select_t(gen)]);
...
int main(){
	int mon_count, i;
	player target_player_dummmy;
	
	target_player_dummmy.set_location(dis(gen), dis(gen));
	...
	for(i=0; i<mon_count; i++){
		cout << endl;
		monster_routine(character_factory::get_monster(i), target_player_dummmy);
	}
