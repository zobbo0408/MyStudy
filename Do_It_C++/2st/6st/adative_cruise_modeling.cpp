...
private:
  ...
  // accelerator 클래스에서 engine 클래스의 멤버를 모두 접근하기 위한 접근 지정자
...
private:
	void acceleration_output() override { increasing_hydrogen(); };
 	void reduce_output() override { decreasing_hydrogen(); };
  	void increasing_hydrogen() { increasing_piston_speed(); };
  	void decreasing_hydrogen() { decreasing_piston_speed(); };
  	void increasing_piston_speed() { cout << "increasing_piston_speed" << endl; };
  	void decreasing_piston_speed() { cout << "decreasing_piston_speed" << endl; };
};
...
private:
  ...
  // break_system 클래스의 pushing_break 함수가 accelator 클래스의 멤버에 접근하기 위해 선언함.
  friend void break_system::pushing_break(accelerator& accelerator_obj);
};

// break_system의 pushing_break 함수 정의
// break_system 클래스의 pushing_break 함수는 accelerator의 레퍼런스 변수(객체)를 매개변수로 받음
// accelerator의 비공개 변수 (private)인 객체를 매개변수로 받을 수 있게 됨.
